...
```rust
let add = |x, y| x + y;
let add2 = add; // 闭包支持copy特性，自身变量不存在所有权问题
println!("{}", add(2,3));
```
...
函数在RUST内也算是一等公民，因为它可以声明在另一个函数内，也可以通过闭包的形式导出，或者把另一个外部函数作为返回值，此外，还能作为入参使用，比如：
...
函数和闭包一样都没有所有权的概念，因此对函数的引用，比如作为入参，或者作为返回值，实际上都是在引用指针。
