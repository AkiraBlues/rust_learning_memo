...
3. 钱包对此哈希值，使用发起方的私钥签名，生成结果R，S，V。
4. 钱包把交易数据连同签名，广播到整个区块链网络
5. 矿工收到广播后，也就拿到了签名，使用发起方公钥解密后可以拿到广播方声明的交易结果的哈希值，然后再把广播的交易数据进行本地哈希，和声明的哈希对比，如果相同，则确认交易的有效性，如果有效，则加入到新区块内
6. 执行实际的资金划转
...
4. 基于私钥加上ECDSA椭圆曲线算法可以得到对应的公钥，这个需要公开到链上
...
函数返回类型，一般用`()`，unit type，表示没有返回值，但还有一种类型，`!`，即never type，表示永远不会返回，写法如下：
...
unit type和never type的区别如下：

- 前者是正常返回，只是没有返回值，相当于写了一个`return;`，执行完成后，CPU可以退出函数，并继续执行后续代码
- 后者是永远不返回，意味着进入这个函数后，不会再出来了，要出来，只能panic!或者人为杀进程
...
SOLANA的介绍，略，有一个playground，在线编程。
