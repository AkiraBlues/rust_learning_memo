...
首先要理解区块链的DDOS，**区块链的设计理念的核心是为了保证所有行为不可篡改**，而DDOS攻击本质上就是希望通过大量的发起交易广播以及确认交易行为，来给链上增加很多没有意义的，类似水军评论那样的交易（在一些区块链设计的社交媒体上，也允许存在水军来刷评论），或者期望能让所有节点都频繁处于更新中，以拖慢整个体系处理正常交易的效率。
...
区块链的钱包，对外而言就是一串字符串，**它表示一个公开的地址，这个地址本质上可以理解为分部式数据库里面的主键**，注意，当在区块链中谈论地址时，基本上就是在谈论公开暴露的地址，即数据主键。很多参与方都有地址，智能合约也会有地址，所以一个地址就是一个存储在数据库内的记录。
...
助记词和私钥最好保存在多份纸质媒体上，并且不要拍照存储，以防止黑客攻击。或者购买硬件钱包。分散资产到不同钱包里也可以降低风险。助记词是最后一道防线，即使私钥丢失了，如果有助记词，还可以恢复，**如果不仅弄丢和私钥，还弄丢了助记词，则没救了**。
...
换言之**单独用let关键字修饰的变量，是在任何条件下都不可变的**，这里先不考虑对象变量的问题，RUST里面的所谓的对象也要遵守相同的规则。**可变性和不可变性，结合借用机制，是RUST能保证运行时无GC且不出BUG的前提，和运行时性能优化没太大关系**，因为RUST没有VM，主要依靠在编译期找出运行时的潜在问题，因此实际上变量是否可变，只要编译通过了，就不会影响到实际运行的性能。
...
不可恢复的异常，比如用下标访问数组导致越界，会导致程序立刻终止。
...
泛型的本质是一种对重复代码逻辑的抽象，只不过提现在接纳不同的数据类型上。RUST的泛型实际上是编译器的语法游戏，即RUST编译器最终会知道某个泛型在编译时可能的类型是什么，这样它就可以在编译时把所有涉及到的具体类型都编译出来，所以最终的机器码提现的实际上是类似枚举的操作，**编译器通过分析代码，找出了所有泛型的真实类型并枚举了出来，这种操作叫做monomorphization，即单态化**。
...
特征作为返回值，这部分后续还会涉及，这里先暂时记住`impl MyTrait`语法。
...
由于RUST所有权的概念，导致开发者倾向于避免所有权频繁转移，所以引用符号`&`会大量使用，但如果引用变量的生命周期长于所有者，那么就会出现空引用的问题。
...
```rust
let add = |x, y| x + y;
let add2 = add; // 部分闭包支持copy特性
println!("{}", add(2,3));
```

上述代码中，通过`|| [expr]`的方式定义了一个匿名函数，并把它保存到变量中，这个匿名函数就是闭包，它和一般的函数不同，入参和返回值都可以不声明类型。**闭包本身如果没有引入外部变量，只是自身形参或者没有形参，那么这个闭包就会自动具有Copy特征，因此可以简单赋值给其他变量，不用考虑所有权问题**。如果闭包引入了外部变量，则闭包本身应该看作是一个拥有了外部变量所有权和计算过程的变量，因此赋值时会发生所有权的转移。

大部分时候闭包的使用场景都是作为其他函数的入参，或者**临时声明一个计算过程，并反复调用**，因此RUST允许大部分时候，省略闭包的入参类型和返回值类型，但也有一些场景是不能省略的，因为此时编译器无法自动推导出类型，所以也可以给闭包加上类型声明：
...
`thread::spawn`会返回一个`JoinHandle`类型的所有权变量，这里翻译为加入主线程的开关（编程术语里面一般会把handle翻译为句柄，或者权柄，不够直接），即这个变量可以让它所属的线程加入到它的主线程，从而延长自身的生命周期。

`join()`方法的解释和使用注意点：

- 作用是**阻塞当前的线程**，等待JoinHandle所属线程执行完成

- 会返回一个Result类型，因为有可能子线程执行过程中会panic!，所以这个Result实际上处理的是子线程panic!的问题，如果子线程正常执行，则一定会加入到主线程中
- **处理不好会导致死锁**，比如主线程持有一个资源，没有释放，子线程需要等待资源释放后才能往下走，如果在主线程释放资源之前，执行了join方法，则导致死锁，主线程在等子线程执行完成，而子线程因为要等主线程释放资源所以也无法完成
...
RUST还支持通过状态共享的方式进行线程间信息传递，和通道相比，状态共享写起来更简单（因为本质上可以看作是所有线程操作同一份数据），但是对开发者有更高要求，需要精心设计。

一般来说，使用通道需要开发者去给不同线程分配不同的通道，假设有5个线程，它们彼此之间都需要通信，那么可能的通道数量就是$4 + 3 + 2 + 1 = 10$，编写起来会很麻烦。而采用状态共享的设计时，只要在关键节点处理好，通过一个共享变量彼此进行通信实际上会更简单。

多线程操作同一份数据，这一命题其实在数据库行业已经有很多研究，RUST采用的是**悲观锁机制**，它的原则如下：

- 共享数据，在任何时间切面上，只能允许单一线程访问和操作
- 线程在访问共享数据之前，需要申请，访问结束后，应释放连接，以便后续其他线程的访问

RUST使用`Mutex`来实现状态共享，名称含义是mutual exclusion（互相排斥，互斥），其实就是悲观锁，用法举例：

```rust
use std::sync::Mutex; // 引入悲观锁
use std::sync::Arc;
...
fn mutex_demo() {
    ...
    if let Ok(mut data) = shared_data.try_lock() {
        data.insert("global_key", String::from("abcdefg")); // data就是锁，需要释放
        println!("[main] data inserted");

        let share_data_2 = Arc::clone(&shared_data); // 把共享数据复制一份，给子线程用
        let child = thread::spawn(move || {
            println!("[child] begin");
            match share_data_2.lock() { // 这里用lock直接阻塞线程，因此注意join的调用时机
                Ok(data) => {
                    println!("[child] lock get");
                    if let Some(key) = data.get("global_key") {
                        println!("get shared value: {key}");
                    }
                },
                Err(e) => {
                    eprintln!("[child] error getting lock, {e:?}");
                }
            }
        });
        thread::sleep(Duration::from_millis(2000)); // 这里模拟主线程缓慢释放锁的过程
        drop(data); // 可以通过代码块来控制锁释放，也可以直接drop
        println!("[main] lock drop");
        child.join().unwrap(); // 主线程释放了锁，可以让它等待子线程完成了，如果在此之前执行join，会导致死锁
    };
    println!("[main] end");
}
```
...
- 用HashMap保存数据，用`Arc<Mutex<HashMap>>`封装为线程间共享对象，之所以要在Mutex外侧再封装一层Arc，是因为需要把共享对象的所有权同时分给主线程和子线程，而`Rc<T>`是不能用于多线程场景的，要用`Arc<T>`代替，它的作用和`Rc<T>`类似都是智能指针，允许一个变量多方持有，也有引用计数
- 在主线程内执行`try_lock`拿到Result类型，里面的Ok值是**MutexGuard类型，这个就是实际的锁，也是共享数据变量**，编写时需要注意它的销毁时机，或者手动去销毁
- 主线程操作共享数据对象，存入信息，**Mutex也实现了内部可变性**，因此虽然传入的HashMap是不可变的，拿到锁之后还是可以修改它
- 把共享数据对象通过`Arc::clone`复制一份，让子线程持有
- 主线程等待一段时间后手动释放锁，之后再执行子线程join方法，此时主线程会等待子线程完成，**过早执行join方法会导致死锁**，因为主线程的锁没有释放，同时等待子线程先完成，而子线程因为没有拿到锁会一直不完成，导致死锁
- 在子线程内通过`lock`方法等待主线程释放锁，并拿到锁，访问对应数据



##### 并发编程相关特征：Send和Sync

这里主要介绍RUST为并发设计的2大特征，Send和Sync，虽然手动实现它们存在风险，但是通过了解可以更清楚地处理编译报错问题。

**Send特征允许一个变量在线程间转移所有权**。之前写过mpsc的通道例子，可以想象**几乎所有RUST内的变量都具有此特征**，因为几乎所有变量都可以使用通道跨越线程。但是还记得`Rc<T>`不支持多线程对吧？所以它也不支持Send特征。原因很简单，Rc变量会记录它的持有者数量，如果A线程持有原本体，B线程持有复制体，那么在计数上会视为A线程也持有复制体，所以在多线程场景下，Rc变量和它的复制体无法精确掌握持有者数量，所以RUST禁止它们跨线程。

**Sync特征允许一个变量被多个线程引用。**注意引用的意思就是`&`，因此**如果一个T类型的变量的引用，即&T，实现了Send特征，即允许引用变量跨线程，则T类型就具有了Sync特征。**`RefCell<T>`不具有Sync特征，因为它跳过了编译期的所有权规则校验，而又没有锁机制，所以在并发场景下会导致修改竞赛的问题，导致某些线程拿到的要么是空指针要么是脏数据，所以禁止多线程使用。`Mutex<T>`通过悲观锁机制确保了实际上就是单线程访问。

手动实现Send和Sync特征风险很大，官方不建议。
...
let gui = GUI {
    cpns: vec![btn, img] // 这里用一个数组包含了2个不同的类型，只不过它们特征相同
};
gui.show();
```
...
默认的所有代码都必须符合RUST内存安全原则，不安全的代码无法通过编译。但实际上**RUST允许开发者编写在编译器看来的内存不安全代码**，因为RUST设计团队也都直到，为了确保内存安全，编译器的规则被设计得比较保守，因此才会有RefCell这样的通过内部可变性绕开所有权限制的指针，这个理念更进一步的实现就是不安全代码块。注意此特性不会让编译器跳过所有的安全检查，即明确会导致问题的代码依然无法编写，编译器只允许在不安全代码块内处理它自身无法明确的场景，包括：
...
不安全代码块让RUST语言更接近硬件层面。

使用`unsafe`代码块开启不安全代码的编写：
...
之前提到过，`impl MyTrait`这种写法也可以用于函数返回值，但是按照之前介绍的OOP的特征知识点，一般会用`Box<dyn MyTrait>`去表示入参类型和返回值类型，这里解释一下。

`impl MyTrait`实际上会要求编译器去推导函数的实际返回类型，因此如果一个函数通过条件控制返回了2个相同特征的不同类型，编译会报错，因为实际上它还是要求编译期函数的返回类型大小已知，比如：
...
impl MyTrait for S1 {}
impl MyTrait for S2 {}
fn return_my_trait() -> impl MyTrait {
    let a = 3;
    if a > 1 {
        S1
    } else {
        S2 // 这里会报错，因为S2和S1是不同类型，也就能推导出编译器实际上把impl MyTrait视为了返回一个固定的类型
    }
}
```

`Box<dyn MyTrait>`则把返回类型固定为了指针，也就因此它的内部实际值可以指向不同的类型，Box类型依然是编译期已知大小的。

特征内部可以定义关联类型，没有数量限制，这里的类型不是具体类型，而是类型名称，或者叫占位符，因此不需要给出实现。类型名称可以用于后续的方法签名，以要求实现方，不仅要实现方法，也要把具体类型套用到类型占位符上，比如：

```rust
pub trait Iterator {
    type Item; // 这里声明了一个关联类型，当然也可以继续声明其他的关联类型
...
RUST在给类型添加特征上，有一个限制，即类型和特征，两者至少要有一个是自定义的，如果全是标准库的，或者三方库的，则无法添加，但是这里有一个绕开手段，前提是类型和特征都不是自定义的，此时可以通过构造一个Wrapper类型，把类型转为自定义的，然后就可以添加三方特征了，比如给`Vec<string>`类型添加`Display`特征，两者都是标准库的，因此给前者包装一下：
...
这种封装方法可以把一些对标准库类型的操作转为更具有业务逻辑的操作，比如对Vec的封装，可以模拟出队列类型或者栈类型等等。
...
一个很长的类型定义可以这样转为精简的别名描述：

```rust
type Trunk = Box<dyn Fn() + Send + 'static>;
```

对于支持泛型的类型，可以用别名使得部分依然是泛型，部分转为具体类型，比如：

```rust
type Result<T> = std::result::Result<T, std::io::Error>; // 这里T就表示OK值的泛型，Err的类型则确定了
```

函数返回类型，一般用`()`，unit type（单元类型），表示没有返回值，但还有一种类型，`!`，即never type，表示永远不会返回，写法如下：
...
unit type（单元类型）和never type的区别如下：

- 前者是正常返回，只是没有返回值，相当于写了一个`return;`，执行完成后，CPU可以退出函数，并继续执行后续代码
- 后者是永远不返回，意味着进入这个函数后，不会再出来了，要出来，只能panic!或者人为杀进程，即阻绝了正常退出的渠道，把这个作为函数返回值，也是提示其他调用者这个函数一旦触发就没有正常退出路径

RUST在编译期需要知道所有变量的类型，因为它需要根据类型确定内存分配大小，而RUST实际上允许一种场景开发者可以不指定类型，此时需要用`?Sized`来表示此变量的类型可能会在编译期未知，由于代码包含的类型都是已知的，因此这部分功能一般都是作为库，开放给外部引入时使用，而且一般也会和泛型结合，即表示泛型变量的类型可能会在编译期未知，例子如下：

```rust
use std::fmt::Debug;

struct Wrapper<T: ?Sized> { // 一个泛型结构体，类型可以在编译期未知
    value: Box<T>, // 对于未知类型的变量，一定要用指针包裹，因为指针大小都是已知的，此外不能是tuple struct
}
...
fn main() {
    let int_wrapper = Wrapper { value: Box::new(42) };
    int_wrapper.print_value(); // 输出42

    let str_wrapper: Wrapper<str> = Wrapper { value: Box::new("Hello, world!") }; // 注意这里的类型是str
    str_wrapper.print_value(); // 输出Hello world
}
```

注意，RUST提供了一个可变大小的类型，叫`str`，虽然写死的字符串都是`&str`类型，但是无法显式声明为`str`类型。此外上述Wrapper结构体，它虽然只有一个成员变量value，但是**不能写成`Wrapper<T: ?Sized>(Box<T>)`，因为tuple结构体必须在编译期确定大小**。



##### 函数 / 闭包进阶

函数在RUST内也算是一等公民，因为它可以声明在另一个函数内（**RUST不建议使用此范式**，因为模块默认所有要素都是私有的，不加pub就无法暴露出去，因此一些自用工具函数也可以直接在根节点声明），也可以通过闭包的形式导出，或者把另一个外部函数作为返回值，此外，还能作为入参使用，比如：
...
fn fn_generator(log_fn: fn() -> ()) -> fn(i32, i32) -> i32 { // 函数作为入参和返回值
    log_fn();
    return add;
}
...
let a = fn_generator(log);
let b = a; // 这里没有所有权转移，说明a是一个函数的指针
let c = a(1,2);
println!("{c}");
```

注意这里的类型是小写的`fn`，它表示函数指针，`Fn`是一个特征，同理，要么用`impl Fn`表示一个固定大小的闭包，要么用`Box<dyn Fn>`去表示一个智能指针，两者**本质上都不是直接返回一个函数**。

RUST无法做到像JS那样真正把函数作为一等公民，即函数内声明一个函数并导出，因为它会导致返回值在编译期未知。所以**RUST只能做到把函数的指针作为入参和返回值，以模拟函数式编程的特性**。这里的`fn`类型，就表示**函数作为一个变量的指针类型**。

函数指针因为它是指针，所以没有所有权的概念。对函数的引用，比如作为入参，或者作为返回值，实际上都是在引用指针，因此要考虑函数的生命周期问题。

用never type，即`!`作为返回值的函数，一般称为Diverging Function，即发散函数，



##### 自引用结构体（结合unsafe代码块，特征和类型进阶，原始指针等内容）
...
macro，宏，之前已经介绍过了，就是元编程，即它本身不是statement或者表达式，而是这些东西的简化，它在编译时会根据传入的参数先动态转为statement或表达式源码，然后再基于这些转为目标OS的机器码。比如`println!`，`dbg!`，`vec!`等调用，都是宏。

RUST里面的宏分为2种，declarative macro（声明式宏）和procedural macro（过程宏），前者通过`macro_rules!`确定，可以视为把`foo!(bar)`直接替换为`Barz::Foo(bar)`，即单纯的文本替换，类似于代码语法糖。后者则更具有侵入性，会按照不同的场景执行不同的代码，也更灵活，比如结构体的特征拓展`#[derive(Debug)]`就是一种宏，显然它不需要`!`符号，但是看上去也和正常的RUST代码有些不同。

元编程的特点就是它比函数更灵活，RUST的函数没有重载，因此一旦声明，其入参类型和入参数量都是固定的，而元编程在这点上更灵活，比如声明式宏`println!("{foo}")`，`println!("a is {}", a)`就支持不同数量和类型的入参。RUST为宏提供的灵活性，使得**声明自定义宏比声明自定义函数需要更复杂的代码，也需要事先声明好后再用，而非像函数那样，调用代码可以在声明代码之前**。

声明式宏，本质是一个基于入参进行模式匹配的操作，以`vec!`声明动态数组举例：
...
上述代码只要看懂核心部分就可以了，比如`vec![a, b, c]`，它是先通过`Vec::new()`构造一个数组，然后遍历并调用`push`方法把匹配到的元素逐个存入，最后返回这个数组。

` #[macro_export]`表示把当前的宏定义暴露出去，这样任何引入此库的代码都可以用到它，也说明宏默认是私有的，和RUST的其他模块设计类似。
...
整体过程和使用match进行模式匹配类似，但不要求像match那样穷尽所有可能性，因为自定义宏本身不存在可穷尽的可能。另外注意到使用了双大括号`{{}}`来表示，这是因为声明式宏在语法上要求返回的必须是一个表达式，因此除非可以用一行完成这个表达式，比如`() => {1}`，否则只能通过内部再加一个代码块的方式来包裹内部的statement，并返回结果，因为代码块本身就是表达式。

然后是表达式写法，使用`&foo: foo_type`表示，这里由于直接面向的是代码，因此类型不再是具体的类型，而是代码的类型，比如：

- expr，表示入参是表达式，一般会用这个
- stmt，statement，表示入参可以是语句指令
- ty，即type，表示入参是类型，比如u8，i32等等
- ident，标识符，这个范围很广，基本上就是任何变量的意思
- tt，最大的范围，表示token，即编程语言里面的最小单元，包括标识符，运算符，各种符号等等，一般不会用这个作为入参类型

然后就是在代码块内针对入参进行处理了，比如一个简单的add宏写法如下：
