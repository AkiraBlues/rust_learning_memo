...
3. A转账给D，2个比特币，那么A需要花费他的UTXO，这里可以选择第一个UTXO，**它持有3个比特币，会被标记为已消费，然后一个新的持有1个比特币的UTXO会被创建，且状态是未消费（unspent）**，他也可以选择消费第二个UTXO，这样这份UTXO就被标记为已消费，第一份UTXO状态保持不变，同时D会拥有一个新的UTXO，金额是2个比特币，状态是未消费
